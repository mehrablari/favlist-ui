...
import AnsweredQuestion from "./questionbox/AnsweredQuestion";
...
const Layout = () => {
  ...
  const checkMatch = () => {
    if(questionMatch === questionId) {
      setGetAnswers(questionMatch)
    }
  }
  ...
  useEffect(() => {
    const fetchAnswers = async () => {
      try {
        const answered = await axios.get(
        ...
        if (answered.status === "FAILED") {
          toast.error(answered.message);
        }
        setIsLoading(false);
        console.log("value in answered",answered.data);
        setAnsweredData(answered.data.data);
        setGetAnswers(answered.data.data.answers);
      } catch (error) {
        console.error(error);
  ...
  return (
    ...
    >
      ...
      {getAnswers ? (<AnsweredQuestion answeredData={answeredData} getAnswers={getAnswers}/>): ( <>
          <Searchbox
          // answerData={apiData}
